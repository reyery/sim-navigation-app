...
public class NavMapFragment extends Fragment implements LocationListener {
    ...
    private boolean mCameraPositionNeedsUpdating = false; // update on first location
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (graphJSON == null) {
            Toast.makeText(getActivity(), "Could not find wayfinding_graph.json from raw resources folder. Cannot do wayfinding.", Toast.LENGTH_LONG).show();
        } else {
            mWayfinder = IAWayfinder.create(getContext(), graphJSON);
        }
    ...
    private void startListeningPlatformLocations() {
        ...
        if (locationManager != null && (ActivityCompat.checkSelfPermission(getContext(), ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED)) {
    ...
    private String loadGraphJSON() {
        try {
            ...
            int resourceIdentifier = res.getIdentifier("wayfinding_graph", "raw", getActivity().getPackageName());
    ...
    public void receiveDest(LatLng point, int floor) {
        if (mMap != null) {
            ...
            if (mWayfinder != null) {
                    mWayfinder.setDestination(point.latitude, point.longitude, floor);
            }
